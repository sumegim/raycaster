...
class RaycastRendererImplementation(RaycastRenderer):
    ...
    def render_brain_regions(self, view_matrix: np.ndarray, volume: Volume, image_size: int, image: np.ndarray):
        # Clear the image
        self.clear_image()
        ...
        for i in range(0, image_size, step):
            for j in range(0, image_size, step):
                ...
                # Normalize value to be between 0 and 1
                max_grad = self.annotation_gradient_volume.get_max_gradient_magnitude()

                shade = 0
                if found:
                    shade = (np.dot(N.reshape(1, 3), L) + 1) / 2

                if 20 < magnitude_f < 100:
                    red = 1
                    green = 0
                    blue = 0
                    alpha = 1 if red > 0 else 0.0
    ...
    def render_mouse_brain(self, view_matrix: np.ndarray, annotation_volume: Volume, energy_volumes: dict,
        ...
        self.render_brain_regions(view_matrix, annotation_volume, image_size, image)
        return
